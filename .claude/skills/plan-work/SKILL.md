...
| CI paths filters                           | `.github/workflows/ci.yml`                 | `server` is `server/src/**` and `client` is `client/src/**` — a `shared/schemas/**` edit reaches **neither** test job. **Editing this file forces the full E2E suite**: plan around it per the gate's "Do not edit `ci.yml`" list, and if a phase still needs an edit, say in the plan which alternative you ruled out                                      |
