...
| A `data-testid` in `client/src`, or an E2E `testId` locator | Run `npm run audit:testids` and commit the regenerated report in the same commit                                                                                                                                                                | `check-testids.ts --check` gates the summary counts and the stale list in `e2e-framework-purity`; a locator that no longer resolves fails the build, and the tracked report goes stale silently otherwise                                                                  |
