 ...
 * root.
 *
 * The description carries a SOURCE path, not a test scope — the resolver
 * matches changed paths against it directly, so a stale glob after a file move
 * is detectable rather than silently selecting nothing.
 */
export interface ImpactsAnnotation {
  type: typeof IMPACTS_ANNOTATION;
  description: string;
}
