...
test(
  'admin creates a text custom field for contacts via Admin Settings @functional',
  async ({ page, restClient, testData }) => {
    ...
    const addForm = await page
      .locate([{ type: 'testId', value: 'add-field-form' }])
      .resolve();
    ...
    }>('/api/custom-fields/definitions?entity_type=contact');
    const created = defsResp.body.definitions.find((d: { id: string; name: string }) => d.name === fieldName);
    expect(created).toBeDefined();
    createdFieldId = created!.id;

    // Register teardown: delete the definition after the test
    testData.register('custom_field_definition', createdFieldId, `/api/custom-fields/definitions/${createdFieldId}`);
  },
);
...
test(
  'rep sets a custom field value on a contact, saves, reloads, confirms persistence @functional',
  async ({ page, restClient, testData }) => {
    await login({ email: ADMIN_EMAIL, password: ADMIN_PASSWORD! }, { page });
    ...
    testData.register('custom_field_definition', definitionId, `/api/custom-fields/definitions/${definitionId}`);
    ...
    const editBtn = await page
      .locate([{ type: 'testId', value: 'edit-contact-button' }])
      .resolve();
...
test(
  'admin deletes a custom field definition; it disappears from the contact detail page @functional',
  async ({ page, restClient, testData }) => {
    await login({ email: ADMIN_EMAIL, password: ADMIN_PASSWORD! }, { page });
    ...
    await restClient.put(`/api/custom-fields/contact/${contact.id}/custom-fields`, [
    ...
    expect(sectionVisible, 'custom fields section should not be visible after definition is deleted').toBe(false);
  },
);
