 ...
 *   - All API count assertions (search.total, page.data.length) are scoped to
 *     UUID-suffixed search terms unique to each test — parallel sibling tests
 *     inserting their own records will not appear in these filtered results.
...
test('@functional F2-C2: optional fields included → all saved and displayed on detail page', async ({
  ...
  const search = await searchContactsViaApi(restClient, lastName);
  expect(search.total, 'contact should be findable via API').toBe(1);
  const id = search.data[0]!.id;
  testData.register('contact', id, `/api/v1/contacts/${id}`);

  const contact = (await getContactById(restClient, id)) as unknown as {
    phone: string;
    title: string;
    department: string;
  };
...
test('@functional F2-C3: missing required field → inline validation, contact not created', async ({
  ...
  const check = await searchContactsViaApi(restClient, uniqueEmail);
  expect(check.total, 'no contact should be created when required field is missing').toBe(0);
...
test('@functional F2-C4: invalid email format → inline validation, contact not created', async ({
  ...
  const check = await searchContactsViaApi(restClient, uniqueLastName);
  expect(check.total, 'no contact should be created after invalid email submit').toBe(0);
...
test('@functional F2-C5: duplicate email address → duplicate warning shown', async ({
  ...
  const search = await searchContactsViaApi(restClient, sharedEmail);
  expect(search.total, 'only one contact with this email should exist').toBe(1);
  expect(search.data[0]!.id, 'the existing contact should be unchanged').toBe(existing.id);
});
...
test('@smoke @functional F2-R1: contact list shows seeded records', async ({
  ...
  const search = await searchContactsViaApi(restClient, `List-${uniqueSuffix}`);
  expect(search.total, 'both seeded contacts should be findable via API').toBe(2);
});
...
test('@functional F2-R2: sort by first name ascending returns alphabetical order', async ({
  ...
  const asc = await listContactsViaApi(restClient, {
    search: `Sort-${uniqueSuffix}`,
    sort: 'first_name',
    dir: 'asc',
  });
  expect(asc.total, 'both contacts should be returned').toBe(2);
  expect(asc.data[0]!.first_name, 'Apple should come first in ascending order').toBe('AppleSort');
  expect(asc.data[1]!.first_name, 'Zebra should come second in ascending order').toBe('ZebraSort');

  // Verify API sort — desc should reverse.
  const desc = await listContactsViaApi(restClient, {
    search: `Sort-${uniqueSuffix}`,
    sort: 'first_name',
    dir: 'desc',
  });
  expect(desc.data[0]!.first_name, 'Zebra should come first in descending order').toBe('ZebraSort');
...
test('@functional F2-R3: sort by email ascending returns correct order', async ({
  ...
  const asc = await listContactsViaApi(restClient, {
    search: `ESort-${uniqueSuffix}`,
    sort: 'email',
    dir: 'asc',
  });
  expect(asc.total, 'both contacts returned').toBe(2);
  expect(asc.data[0]!.email, 'aaa email should come first in ascending order').toMatch(/^aaa-/);
  ...
  expect(desc.data[0]!.email, 'zzz email should come first in descending order').toMatch(/^zzz-/);
});
...
test('@functional F2-R4: search matching name returns results (AC3 — case-insensitive)', async ({
  ...
  const apiSearch = await searchContactsViaApi(restClient, `CIS-${uniqueSuffix}`.toUpperCase());
  expect(apiSearch.total, 'API search should also be case-insensitive').toBeGreaterThan(0);
  expect(apiSearch.data.some((c) => c.id === contact.id)).toBe(true);
});
...
test('@smoke @functional F2-U1: edit first name → change reflected in detail view and list', async ({
  ...
  const updated = await getContactById(restClient, contact.id);
  expect(updated.first_name, 'first name should be updated').toBe(updatedFirst);
});
...
test('@functional F2-U2: edit last name → change reflected in detail view and list', async ({
  ...
  const updated = await getContactById(restClient, contact.id);
  expect(updated.last_name, 'last name should be updated').toBe(updatedLast);
});
...
test('@functional F2-U3: cancel edit → no changes persisted', async ({
  ...
  const unchanged = await getContactById(restClient, contact.id);
  expect(unchanged.first_name, 'first name must not change after cancel').toBe('CancelEditOrig');
});
...
test('@functional F2-D2: cancel confirmation dialog → contact not deleted, remains in list', async ({
  ...
  // Confirm contact still exists in the API (getContactById throws on non-2xx).
  await getContactById(restClient, contact.id);
});
...
test('@functional F2-A1: link contact to account → contact appears in account contacts list', async ({
  ...
  await patchContactAccount(restClient, contact.id, account.id, contact.version);
  ...
  const detail = await getContactById(restClient, contact.id);
  expect(detail.account_id, 'contact account_id should match').toBe(account.id);
});
...
test('@functional F2-A2: unlink contact from account → account_id is null in API', async ({
  ...
  const before = await getContactById(restClient, contact.id);
  expect(before.account_id, 'contact should be linked before unlink').toBe(account.id);
  ...
  // Verify via API.
  const after = await getContactById(restClient, contact.id);
  expect(after.account_id, 'account_id should be null after unlink').toBeNull();
});
...
test('@functional F2-P1: pagination — navigating pages returns correct records (AC2 — sort stable)', async ({
  ...
  const page1 = await listContactsViaApi(restClient, {
    search: `Page-${uniqueSuffix}`,
    sort: 'first_name',
    dir: 'asc',
    limit: 2,
    page: 1,
  });
  expect(page1.total, 'total should be 3').toBe(3);
  expect(page1.data.length, 'page 1 should have 2 records').toBe(2);
  expect(page1.data[0]!.first_name, 'first on page 1 should be Pag00').toBe('Pag00');
  expect(page1.data[1]!.first_name, 'second on page 1 should be Pag01').toBe('Pag01');

  // AC2: page 2 must maintain the same sort order.
  const page2 = await listContactsViaApi(restClient, {
    search: `Page-${uniqueSuffix}`,
    sort: 'first_name',
    dir: 'asc',
    limit: 2,
  ...
  expect(page2.data.length, 'page 2 should have 1 record').toBe(1);
  expect(page2.data[0]!.first_name, 'record on page 2 should be Pag02').toBe('Pag02');
