...
export class ProfilePage {
  ...
  /** Returns true when the connected mailboxes panel is visible. */
  async connectedAccountsSectionIsVisible(): Promise<boolean> {
    try {
      const resolved = await this.page
        .locate(
          [
            { type: 'testId', value: 'connected-accounts-section' },
            { type: 'css', value: '[data-testid="connected-accounts-section"]' },
          ],
          { intent: 'connected mailboxes panel on the profile page' },
        )
        .resolve();
      return resolved.isVisible();
    } catch {
  ...
  /** Returns true when the panel shows its no-mailboxes-yet message. */
  async connectedAccountsAreEmpty(): Promise<boolean> {
    try {
      const resolved = await this.page
        .locate(
          [
            { type: 'testId', value: 'connected-accounts-empty' },
            { type: 'css', value: '[data-testid="connected-accounts-empty"]' },
          ],
          { intent: 'message shown when no mailbox is connected yet' },
        )
        .resolve();
      return resolved.isVisible();
    } catch {
  ...
  /** Reveals the IMAP credentials form. */
  async clickConnectViaImap(): Promise<void> {
    await this.page.click(
      [
        { type: 'testId', value: 'connected-accounts-show-imap-form' },
        { type: 'role', value: 'button', options: { name: /imap/i } },
      ],
      { intent: 'button revealing the IMAP mailbox credentials form' },
    );
  }

  /** Types a value into one field of the IMAP credentials form. */
  async fillImapField(
    field: 'email' | 'host' | 'port' | 'username' | 'password',
    value: string,
  ): Promise<void> {
    await this.page.fill(
      value,
      [
        { type: 'testId', value: `connected-accounts-imap-${field}` },
        { type: 'css', value: `[data-testid="connected-accounts-imap-${field}"]` },
      ],
      { intent: 'a credential field in the IMAP mailbox form' },
    );
  }

  /** Submits the IMAP credentials form. */
  async clickImapSubmit(): Promise<void> {
    await this.page.click(
      [
        { type: 'testId', value: 'connected-accounts-imap-submit' },
        { type: 'role', value: 'button', options: { name: /connect/i } },
      ],
      { intent: 'submit button of the IMAP mailbox form' },
    );
  }

  /** Returns the message shown when a mailbox could not be connected. */
  async connectErrorText(): Promise<string | null> {
    const resolved = await this.page
      .locate(
        [
          { type: 'testId', value: 'connected-accounts-connect-error' },
          { type: 'css', value: '[data-testid="connected-accounts-connect-error"]' },
        ],
        { intent: 'error shown when a mailbox could not be connected' },
      )
      .resolve();
    return resolved.textContent();
  }

  /** Returns the destination of one provider's connect button. */
  async oauthConnectHref(provider: 'google' | 'microsoft'): Promise<string | null> {
    const resolved = await this.page
      .locate(
        [
          { type: 'testId', value: `connected-accounts-connect-${provider}` },
          { type: 'css', value: `[data-testid="connected-accounts-connect-${provider}"]` },
        ],
        { intent: 'link starting an OAuth mailbox connection' },
      )
      .resolve();
