...
describe('getDashboardSummary — deal aggregates', () => {
  ...
  it('counts open deals and sums their value', async () => {
    ...
    await pool.query(
      `INSERT INTO deals (name, stage, value, owner_id, pipeline_id, pipeline_stage_id)
       VALUES ('Deal A', 'Prospecting', 25000, $1, $2, $3),
              ('Deal B', 'Qualification', 75000, $1, $2, $4)`,
      [repId, defaultPipelineId, stageIdProspecting, stageIdQualification],
    );
    const summary = await getDashboardSummary(repId);
  ...
  it('excludes Closed Won and Closed Lost deals from metrics', async () => {
    ...
    await pool.query(
      `INSERT INTO deals (name, stage, value, owner_id, pipeline_id, pipeline_stage_id)
       VALUES ('Open deal', 'Prospecting', 10000, $1, $2, $3),
              ('Won deal', 'Closed Won', 50000, $1, $2, $4),
              ('Lost deal', 'Closed Lost', 20000, $1, $2, $5)`,
      [repId, defaultPipelineId, stageIdProspecting, stageIdClosedWon, stageIdClosedLost],
    );
    const summary = await getDashboardSummary(repId);
  ...
  it('scopes deal counts to the given owner when ownerId is provided', async () => {
    ...
    await pool.query(
      `INSERT INTO deals (name, stage, value, owner_id, pipeline_id, pipeline_stage_id)
       VALUES ('Rep deal', 'Prospecting', 10000, $1, $3, $4),
              ('Other deal', 'Qualification', 20000, $2, $3, $5)`,
      [repId, otherRepId, defaultPipelineId, stageIdProspecting, stageIdQualification],
    );
    const summary = await getDashboardSummary(repId);
    expect(summary.openDealCount).toBe(1);
  });
...
describe('getDashboardSummary — stage breakdown', () => {
  it('returns a breakdown row for each open stage', async () => {
    ...
    await pool.query(
      `INSERT INTO deals (name, stage, value, owner_id, pipeline_id, pipeline_stage_id)
       VALUES ('Deal 1', 'Prospecting', 10000, $1, $2, $3),
              ('Deal 2', 'Prospecting', 15000, $1, $2, $3),
              ('Deal 3', 'Qualification', 30000, $1, $2, $4)`,
      [repId, defaultPipelineId, stageIdProspecting, stageIdQualification],
    );
    const summary = await getDashboardSummary(repId);
  ...
  it('does not include closed stages in the breakdown', async () => {
    ...
    await pool.query(
      `INSERT INTO deals (name, stage, value, owner_id, pipeline_id, pipeline_stage_id)
       VALUES ('Won deal', 'Closed Won', 50000, $1, $2, $3)`,
  ...
  it('handles deals with null value in stage breakdown', async () => {
    const stageIdProposal = (
      await pool.query<{ id: string }>(
        'SELECT id FROM pipeline_stages WHERE name = $1 AND pipeline_id = $2 LIMIT 1',
        ['Proposal', defaultPipelineId],
      )
    ).rows[0].id;
    await pool.query(
      `INSERT INTO deals (name, stage, value, owner_id, pipeline_id, pipeline_stage_id)
       VALUES ('No value deal', 'Proposal', NULL, $1, $2, $3)`,
      [repId, defaultPipelineId, stageIdProposal],
    );
    const summary = await getDashboardSummary(repId);
...
describe('getDashboardSummary — weighted pipeline value', () => {
  it('returns weightedPipelineValue using stage default probability', async () => {
    ...
    await pool.query(
      `INSERT INTO deals (name, stage, value, owner_id, pipeline_id, pipeline_stage_id)
       VALUES ('Weighted test', 'Prospecting', 100000, $1, $2, $3)`,
  ...
  it('uses per-deal probability override for weighted calculation', async () => {
    ...
    await pool.query(
      `INSERT INTO deals (name, stage, value, probability, owner_id, pipeline_id, pipeline_stage_id)
       VALUES ('Override deal', 'Prospecting', 100000, 60, $1, $2, $3)`,
      [repId, defaultPipelineId, stageIdProspecting],
    );
    const summary = await getDashboardSummary(repId);
  ...
  it('includes weighted_value in per-stage breakdown', async () => {
    ...
    await pool.query(
      `INSERT INTO deals (name, stage, value, owner_id, pipeline_id, pipeline_stage_id)
       VALUES ('Stage weighted', 'Qualification', 80000, $1, $2, $3)`,
  ...
  it('returns 0 weighted value when all deals have null value', async () => {
    ...
    await pool.query(
      `INSERT INTO deals (name, stage, value, owner_id, pipeline_id, pipeline_stage_id)
       VALUES ('No value', 'Prospecting', NULL, $1, $2, $3)`,
      [repId, defaultPipelineId, stageIdProspecting],
    );
    const summary = await getDashboardSummary(repId);
  ...
  it('excludes closed stages from weighted pipeline total', async () => {
    ...
    await pool.query(
      `INSERT INTO deals (name, stage, value, owner_id, pipeline_id, pipeline_stage_id)
       VALUES ('Won deal', 'Closed Won', 100000, $1, $2, $3)`,
      [repId, defaultPipelineId, stageIdClosedWon],
    );
    const summary = await getDashboardSummary(repId);
