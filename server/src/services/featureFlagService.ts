...
/** Raw DB row from the feature_flag_groups table joined with updated_by name. */
...
async function getCachedRows(): Promise<FeatureFlagDbRow[]> {
  ...
  // so a scheduled enable fires without waiting for the full 60-second TTL.
  const now = Date.now();
 ...
 * A row is schedule-enabled when enable_at is set and is in the past or present.
 */
 ...
 * Accounts for enable_at scheduled enables.
 ...
 * falls back to the org-wide enabled value.
 *
...
export async function isFlagEnabledForRole(key: string, role: string): Promise<boolean> {
  ...
  // A scheduled enable supersedes everything — treat as fully enabled.
...
async function assertValidRoleOverrides(
  ...
  // TOCTOU race while avoiding unnecessary locks on unrelated rows.
...
export async function updateFeatureFlag(
  ...
  try {
    ...
    // check is consistent with the state visible to the concurrent UPDATE below.
    ...
    // Validate that the target group exists when assigning.
    ...
    // Audit enable_at changes — compare as ISO strings to normalise timezone representation.
    ...
    // Audit rollout_percentage changes.
    if (
    ...
    // Audit group assignment changes.
...
// is implemented below in the Beta Users section. These count helpers are used
// by listFeatureFlags/getFeatureFlag above.

/**
 * Returns the count of users enrolled in the beta for a specific flag.
 * Always queries fresh — beta membership is never served from the flag cache.
 */
...
export async function isFlagEnabledForUser(
  ...
  // all other targeting rules including group gates")
  ...
  // (e.g. ai_lead_routing_suggestion). Loses to a per-user force override
  ...
  // The group gate is a kill switch for all non-force-overridden users.
  if (row?.group_key) {
  ...
  // Step 3: flag-level beta membership → always enabled for beta users.
 ...
 * Throws a typed BETA_USER_ALREADY_ENROLLED error on duplicate (PG 23505).
 *
 ...
 * Writes an audit log entry in the same transaction.
 * Returns false if no enrollment existed.
 */
 ...
 * A user may have at most one override per flag — an existing override is replaced.
...
/**
 * Creates a new flag group. Throws FLAG_GROUP_DUPLICATE_KEY on 23505.
 */
 ...
 * for the group deletion itself, all within the same transaction.
 */
 ...
 * Writes an audit entry in the same transaction.
 * Throws GROUP_BETA_USER_ALREADY_ENROLLED on duplicate.
 */
...
export async function advanceRolloutStages(actor: AuditActor): Promise<void> {
  ...
  for (const { flag_key } of candidatesResult.rows) {
    ...
    try {
      ...
      // on this flag, we skip it — it will advance the stage instead.
 ...
 * Call stopRolloutScheduler() on process shutdown.
 */
