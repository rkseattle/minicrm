...
export async function seedDemo(): Promise<{ seeded: boolean; reason?: string }> {
  ...
  // The password is a constant in this file and the token is written to
  // .env.demo below; neither belongs in a log stream that staging also emits.
  logger.info({ email: DEMO_REP.email }, 'Demo data seeded');
  ...
  logger.info({ path: envDemoPath }, 'Demo service-account token written');
...
export async function resetDemo(): Promise<{ reset: boolean }> {
  ...
  logger.info('Demo service-account token regenerated');
  const envDemoPath = path.resolve(process.cwd(), '.env.demo');
  await fs.writeFile(
    envDemoPath,
    `# Auto-generated by npm run seed:demo — do not commit\nDEMO_SVC_API_TOKEN=${svcToken}\n`,
