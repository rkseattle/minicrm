...
| `POST` | `/api/v1/admin/coverage/sessions/:sessionId/dumps` | Record a `dumpId`'s attribution to a session (after `POST /coverage/dump`). `409` if the session has already ended, or if `dumpId` was already recorded (anywhere — including via auto-attribution above). |
...
A dump can only ever attribute to an **active** session — `recordCoverageSessionDump`'s INSERT is scoped to `WHERE EXISTS (... status = 'active')` atomically, so there's no check-then-insert race, and the attempt fails with `409 COVERAGE_SESSION_ENDED` rather than silently succeeding against a closed session.
