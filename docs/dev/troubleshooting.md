...
resolve against. `SKIP_TIA_PREPUSH=1 git push` skips **only** the E2E leg — typecheck and the
audit gate still run, and every use is appended to the local
`.git/tia-prepush-bypass.log`. Reaching for it because the suite is slow pushes
E2E-unverified; the one case where it does not is a hook run that already failed and whose
cause you fixed and re-ran narrowly, per "After the hook's own E2E run fails" in
