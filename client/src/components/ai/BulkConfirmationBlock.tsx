...
export default function BulkConfirmationBlock({
  ...
  // Confirm is only allowed when user types the count number or the word "DELETE"
  const isConfirmAllowed =
    confirmText === String(bulkCount) || confirmText.toUpperCase() === 'DELETE';

  return (
    <div data-testid="nli-bulk-confirmation-block">
      ...
      {/* Warning callout — rendered above the field table area */}
      <div className="mt-3 border border-red-200 bg-red-50 border-s-4 border-s-red-500 rounded-lg ps-4 pe-4 py-3">
          ...
          <div className="mb-3">
            ...
            {pendingAction.bulkSample && pendingAction.bulkSample.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2" data-testid="nli-bulk-sample">
                {pendingAction.bulkSample.map((name: string) => (
                  <span
                    key={name}
